 ...
 * This is used by the CALIBRAGE example file
 */
var xPast50 = new Array(50);
var yPast50 = new Array(50);

/**
 * Variable qui stocke toutes les prédictions, en attendant de les télécharger sous forme d'un fichier
 */
var predictionArray = [];
var nbPred = 0; // nombre de prédictions enregistrées
var luminanceArray = []; // 
 ...
 */
function drawCoordinates(colour,x,y, size=5){
...
function getPupilFeatures(canvas, width, height) {
  ...
  try {
    // TODO : faire les étapes ici quand on récupère les données de luminance (récup lumi moyenne dans patches)
    const patches = curTracker.getEyePatches(videoElement, canvas, width, height);
    return patches;
  } catch(err) {
...
async function loop() {
  if (!paused) {
    ...
    if( latestGazeData ) {
      ...
      // Saving of gaze predictions
      const recordingState = recording;
      const savingState = saving;
      if (recordingState) {
          predictionArray.push([Math.round(pred.x), Math.round(pred.y), latestGazeData.eyeFeatures.timestamp]);
          luminanceArray.push(latestGazeData.eyeFeatures.luminance);
          nbPred++;
      }
      // TODO : déclencher le changement de cette variable quand le jeu renvoie qu'il est fini
      if (savingState) {
        saveGazeRecording(predictionArray, luminanceArray, nbPred);
        // variables reset in the case we start a new recording later in the session
        nbPred = 0;
        predictionArray = [];
        luminanceArray = [];
        stopSaving();
      ...
      if (webgazer.params.storingPoints) {
        drawCoordinates('skyblue',pred.x,pred.y); //draws the previous predictions
...
var moveListener = function(event) {
  ...
  if (now < moveClock + webgazer.params.moveTickSize) { // moveTickSize = 50, probablement le nombres de frames à record
    return;
  } else {
    moveClock = now;
  }
...
webgazer.begin = function(onFail) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      // TODO : Changer cette merde fissa. Autre bidouillage atroce (faire passer les dimensions webcam dans la partie non minifiée du code)
      // Analysis tool: not intended to last in the prod tracker
      const camWidth = document.createElement('div');
      const camHeight = document.createElement('div');
      camWidth.style.display = 'none';
      camHeight.style.display = 'none';
      const streamSettings = stream.getVideoTracks()[0].getSettings();
      camWidth.textContent = streamSettings.width;
      camHeight.textContent = streamSettings.height;
      camWidth.id = 'camWidth';
      camHeight.id = 'camHeight';
      document.body.appendChild(camWidth);
      document.body.appendChild(camHeight);
...
/**
 * Set whether previous CALIBRAGE data (from localforage) should be loaded.
 * Default true.
 *
...
/*
 * Stores the position of the fifty most recent tracker predictions
 */
